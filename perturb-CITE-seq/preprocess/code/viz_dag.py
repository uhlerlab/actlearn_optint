...
def draw(pdag, node_names=None):
    ...
    nx.draw(e, pos=pos, node_color='w', ax=nw_ax, edge_color='red')
    nx.draw(d, pos=pos, node_color='w', ax=nw_ax)
    if node_names is not None:
        labels={node: node_names[node] for node in range(p)}
    else:
        labels={node:node for node in pdag.nodes}
    nx.draw_networkx_labels(d, pos, labels=labels, ax=nw_ax, font_size=8)